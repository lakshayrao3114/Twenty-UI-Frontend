...
export default function Tasks() {
    ...
    const getandSetTodos = () => {
        ...
        var todos: any = [];
        JSON.parse(localStorage.getItem('people') || '')?.forEach(todo => {
    ...
    const handleTaskStatus = (id: any, name: String) => {
        const newData: any = JSON.parse(localStorage.getItem('people')).map((item: any) => {
            if(item?.id === id){
        ...
        <div className="w-full border h-[600px] rounded-md">
            <Tabs  defaultValue="todo">
                ...
                <TabsContent className="p-2" value="todo">
                    <div className="mx-1 text-lg font-bold mb-2">
                        Upcoming
                    </div>
                    <ul className="mx-1">
                        {todos?.filter((todo: any) => todo.status != 1).map((task: any, index: any) => (
                            <li className="border p-2 mt-1 flex items-center" key={index}>
                                <div>{task.name}</div>
                                {task.status === 0 &&
                                    <div className="ml-auto">
                                        <Button onClick={(e) => handleTaskStatus(task.id, task.name)}>Mark as done</Button>
                                    </div>}
                ...
                <TabsContent className="p-2" value="done">
                    <div>
                    <ul className="mx-1">
                        {todos?.filter((todo: any) => todo.status != 0).map((task: any, index: any) => (
                            <li className="border p-2 mt-1 flex items-center" key={index}>
                                <div>{task.name}</div>
                                {task.status === 0 &&
                                    <div className="ml-auto">
